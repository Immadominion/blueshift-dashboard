...
Your wallet contains $10,000. An attacker roots your phone and extracts the encrypted keystore. With offline brute-force attacks, they have unlimited time to crack it. Once compromised, everything is lost.

Traditional key storage creates binary outcomes. Either the attacker has your key or they don't. No middle ground exists. Device compromise means total fund loss. Backup theft means total fund loss. A single vulnerability anywhere in the chain compromises everything.

Multi-Party Computation (MPC) changes the equation. The private key is split into shares distributed across multiple parties. No single party ever holds the complete key. To sign a transaction, shares collaborate through cryptographic protocols that produce valid signatures without ever reconstructing the original key. An attacker must compromise multiple independent systems simultaneously.
...
From a developer perspective, you interact with this through the provider's SDK. The MPC protocol runs transparently:

```typescript
// Request signature through embedded wallet SDK
const signature = await wallet.signTransaction(transaction);

// Behind the scenes:
// 1. Device share is unlocked with biometric
// 2. SDK sends signing request to provider
// 3. Provider authenticates the request
// 4. MPC protocol executes between device and HSM
// 5. Valid Ed25519 signature returned
// The private key never existed in whole at any point
```
...
This is dramatically harder than compromising a single location. The API abstraction hides this complexity, presenting a standard wallet interface while MPC operates underneath.
