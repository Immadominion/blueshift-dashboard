...
A user receives a convincing email. They click the link to `evil-myapp.com` and enter their password. The attacker now controls their account. This attack—phishing—succeeds because passwords are transmittable secrets. Once revealed, they work anywhere.

Passkeys eliminate this vulnerability. Every passkey is cryptographically bound to its origin domain. A passkey created for `myapp.com` refuses to sign for `evil-myapp.com`, even if the user attempts it. The binding happens in hardware. No amount of social engineering can override it. Phishing becomes mathematically impossible.
...
Instead of a traditional keypair wallet, the "wallet" becomes a PDA derived from the passkey's public key. The passkey signature directly authorizes on-chain operations without any intermediary custody.
...
```typescript
// User initiates transfer on device
const transferIx = SystemProgram.transfer({
  fromPubkey: walletPDA,  // The PDA derived from passkey pubkey
  toPubkey: recipient,
  lamports: amount,
});

// Create challenge from transaction data
const challenge = Buffer.from(transferIx.data);

// Request passkey signature (triggers FaceID/fingerprint)
const passkeySignature = await navigator.credentials.get({
  publicKey: {
    challenge,
    rpId: 'myapp.com',
...
// Submit to on-chain program with passkey signature
const tx = new Transaction().add(
  // Instruction containing passkey signature + challenge
  createVerifyAndExecuteInstruction({
    passkeySignature,
    challenge,
...
The on-chain program verifies the passkey signature proves ownership of the wallet PDA, then executes the transfer. Without the passkey, the PDA cannot be accessed—no seed phrase, no private key file, just hardware-bound authentication.
...
Passkeys eliminate phishing attacks through origin binding, prevent credential theft through public-key cryptography, and remove keylogger vulnerabilities by avoiding typed secrets—but aren't without considerations.
