...
This reduces transaction approval latency from 2-3s per passkey signature to <100ms per session key signature without compromising security. Games can execute rapid transactions with session keys while high-value operations still require passkey authorization.
...
LazorKit includes paymaster support for [gas abstraction](#account-abstraction):
...
The paymaster wraps user-signed operations into fee-paid transactions. Users interact with Solana applications without needing SOL. See the [Account Abstraction Patterns](#account-abstraction) section above for how gas sponsorship and alternative fee tokens work.
...
- High-value? Need multiple recovery paths: social recovery, backup authentication, or provider-assisted recovery.
- Disposable? Single recovery method or no recovery acceptable.
...
3. **[Gas abstraction](#account-abstraction)** lets users interact without holding SOL through paymasters
