...
This next-generation standard employs a single account design that reduces minting costs from ~0.012 SOL (Token Metadata's 3 accounts) to ~0.0035 SOL (Core's 1 account) while reducing account lookup overhead through its flexible plugin system.
