...
Single RPC dependencies create cascading failures. When your primary provider hits rate limits or goes down, every transaction in your app fails simultaneously. Multiple providers with automatic failover ensure continuity.
...
The modulo operator in `rotateEndpoint` ensures cycling through the array wraps back to the first endpoint. The error message checks detect rate limiting (429) and network timeouts, triggering rotation before retrying. Without this pattern, a single provider outage becomes a complete app outage.

### Rate limiting awareness

RPC providers have rate limits. Handle them gracefully:

When you hit rate limits, immediately retrying compounds the problem. Exponential backoff spaces out retries, giving the provider's rate limiter time to reset while preventing thundering herd scenarios where many clients retry simultaneously.
...
The exponential backoff (2^attempt) means delays grow from 1s to 2s to 4s to 8s, with a 30s cap. The `Math.min` prevents infinite growth that could freeze your UI. Without the cap, attempt 15 would wait over 9 hours.

### WebSocket health

For real-time updates, WebSocket connections can silently fail:

WebSocket disconnections often happen without error events. Network changes, load balancer timeouts, or RPC provider restarts can leave your subscription active in memory but dead in practice. Users see stale data until they restart the app.
...
The 30-second health check calls `getSlot()` over the same WebSocket connection. If it fails, the connection is dead and gets recreated. Without this, a silent disconnection means users never see balance updates, transaction confirmations, or other real-time events.
...
```typescript
// Reduce RPC costs by minimizing request frequency

// Bad: Polling every second (86,400 requests/day per user)
setInterval(() => connection.getBalance(publicKey), 1000);

// Good: Use WebSocket subscriptions (1 subscription per user, unlimited updates)
connection.onAccountChange(publicKey, (accountInfo) => {
  updateBalance(accountInfo.lamports);
});

// Best: Batch multiple independent queries into parallel requests (3 separate requests → 1 roundtrip)
...
This completes the final course in the Solana mobile development curriculum. Here's what we covered:
