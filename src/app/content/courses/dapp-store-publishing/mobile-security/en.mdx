...
Every APK you ship is readable source code. The `strings` command extracts hardcoded API keys in seconds. Tools like `jadx` decompile your entire application back to Java. Attackers don't need sophisticated techniques—just standard tools and five minutes.

This creates constraints web developers don't face. Server-side secrets stay on servers. Client-side secrets live on devices you don't control, in files attackers can freely examine. Understanding what you can and cannot protect determines your security architecture.
