...
The CLI requires Node.js 18-21 because it depends on native cryptographic libraries (ed25519 and secp256k1) that only compile cleanly against these versions. Node 22+ changed internal APIs that break compatibility with the Solana web3.js dependency chain.
...
This creates a `config.yaml` with the proper structure for your app metadata. If `node --version` shows 22 or higher, use a version manager (nvm/fnm) to switch to Node 20 before proceeding.
...
These size requirements match standard Android asset guidelines and ensure your app displays correctly across all Solana Mobile Stack devices, from the Saga to future hardware.
...
The 512×512 icon size is Android's launcher icon baseline. The 1200×600 banner follows the 2:1 aspect ratio used for featured content across mobile stores. The 1080px minimum for screenshots ensures quality on modern high-DPI displays without requiring unnecessarily large files.
...
With everything prepared, create your Publisher identity.
...
Publishing to the Solana dApp Store means your app registry entry is an NFT you control. No corporation can delist you without your keys. The review process exists to protect users, not extract rent. Direct crypto payments work because the platform treats blockchain interaction as a feature, not a violation.

This distribution model is fundamentally different from traditional app stores. The next challenge is navigating Apple and Google's crypto policies while maintaining feature parity. Traditional app stores treat blockchain interaction with suspicion; success requires understanding their specific constraints and red lines.
