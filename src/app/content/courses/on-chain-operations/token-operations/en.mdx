...
3. Sign with your wallet (MWA's `wallet.signAndSendTransactions()` or embedded wallet provider methods)
4. Send and confirm

The instruction building and transaction construction code is identical across wallet types. Only step 3 varies based on whether you use Mobile Wallet Adapter or embedded wallets.
...
Token operations on mobile require more care than desktop. Every transfer might create a token account, costing SOL the user didn't expect to pay. Decimal precision errors compound across transactions, making precise math non-negotiable. The Token-2022 program introduces extension-specific logic that breaks assumptions from the original Token Program.

These patterns—checking for token accounts before building transactions, handling program IDs correctly, showing multi-step progress—separate mobile token apps that feel professional from those that frustrate users with unexpected failures.
