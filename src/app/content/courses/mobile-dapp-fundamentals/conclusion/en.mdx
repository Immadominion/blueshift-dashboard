...
You understand MWA from protocol fundamentals to production patterns. You know why ephemeral sessions exist, how ECDH key exchange secures communication, and how auth tokens enable frictionless reconnection. The protocol's architectural choices—local socket transport, stateless sessions, explicit authorization—all follow from mobile's security constraints.

The patterns here—AuthorizationProvider, error handling, device testing—handle the messy reality of mobile development. Network failures, user cancellations, wallet crashes, and version mismatches happen constantly. Production apps survive these conditions by building robustness into every interaction.

<ArticleSection name="Next Steps" id="next-steps" level="h2" />

With MWA fundamentals complete, you can build production mobile dApps that connect to external wallets. The curriculum branches from here based on your needs.

**Course 2: Embedded Wallets** teaches an alternative to MWA where the wallet lives inside your app rather than as an external dependency. Users authenticate with biometrics or social login instead of managing seed phrases. Many production apps implement both approaches: embedded wallets for <1s wallet creation without seed phrases, MWA for users who prefer self-custody.

**Course 3: On-Chain Operations** covers what happens after wallet connection: token transfers, NFT operations, program interaction, and Blinks. The operations work identically whether you use MWA or embedded wallets.

**Course 4: MWA Deep Dive** explains the protocol internals. Most developers won't need this level of detail. Take it if you're building a wallet, debugging connection failures, conducting security audits, or implementing custom transports like Bluetooth.

**Course 5: dApp Store Publishing** walks through distribution: publishing to the Solana dApp Store, navigating Apple and Google app store policies for crypto apps, and production best practices.

After Course 1, Courses 2, 3, and 4 can be taken in any order. Course 5 assumes you have a working app from Courses 1 and 3.
