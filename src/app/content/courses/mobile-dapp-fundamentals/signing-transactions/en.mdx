...
This lesson covers creating utility functions for building transactions and a `useSendSol` hook that handles the complete send flow.
...
The wallet shows users all transactions in a single approval prompt. This reduces user friction from 3 approval prompts to 1.
...
Transaction signing is where theory meets user intent. Every pattern here—fresh blockhashes, signature confirmation, error handling—exists because distributed systems have no concept of "undo." Unlike web forms that can be resubmitted or retried freely, blockchain transactions are final the moment they confirm. This forces upfront rigor in a way web development rarely requires, and understanding this changes how you think about user actions in any decentralized context.
